...
public class FlickrHTTPClient: NSObject {
    ...
    var delegate:FlickrHTTPClientProtocol!
    ...
    convenience init(delegate:FlickrHTTPClientProtocol) {
        self.init()
    ...
    func taskWithBodyMethod(httpMethod:String, method: String, parameters: [String:AnyObject], jsonBody: [String:AnyObject], completionHandler: (result: AnyObject!, error: NSError?) -> Void) -> NSURLSessionDataTask {
        ...
        let urlString = self.delegate.getBaseURLSecure() + method + FlickrHTTPClient.escapedParameters(parameters)
        ...
        let task = session.dataTaskWithRequest(request) { data, response, downloadError in
            ...
            if let error = downloadError {
                let newError = FlickrHTTPClient.errorForData(newData, response: response, error: error)
                completionHandler(result: nil, error: newError)
            } else {
                FlickrHTTPClient.parseJSONWithCompletionHandler(newData, completionHandler: completionHandler)
            }
        }
    ...
    func taskForGETMethod(method:String, parameters:[String:AnyObject], completionHandler: (result:AnyObject!, error:NSError?) -> Void) -> NSURLSessionDataTask {
        ...
        let urlString:String! = self.delegate.getBaseURLSecure() + method + FlickrHTTPClient.escapedParameters(parameters)
        let url = NSURL(string: urlString)!
        let request = NSMutableURLRequest(URL: url)
        self.delegate.addRequestHeaders(request)
    ...
    class func errorForData(data: NSData?, response: NSURLResponse?, error:NSError) -> NSError {
        ...
        if let parsedResult = (try? NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions.AllowFragments)) as? [String:AnyObject] {
            if let errorMessage = parsedResult[FlickrClient.JSONResponseKeys.ErrorMessage] as? String {
                let userInfo = [NSLocalizedDescriptionKey: errorMessage]
                if let errorCode = parsedResult[FlickrClient.JSONResponseKeys.Status] as? Int {
